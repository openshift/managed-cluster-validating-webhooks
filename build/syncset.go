					...
					Containers: []corev1.Container{
						{
							...
							Name:            "webhooks",
							// REPO_DIGEST is generated by app-sre
							Image: "${REPO_DIGEST}",
...
func main() {
	...
	te := templatev1.Template{
		...
		Parameters: []templatev1.Parameter{
			...
			// - required to generate REPO_DIGEST
			{
				Name:     "IMAGE_TAG",
				Required: true,
			},
			...
			// REGISTRY_IMG is required to generate REPO_DIGEST
			{
				Name:     "REGISTRY_IMG",
				Required: true,
				Value:    fmt.Sprintf("quay.io/app-sre/%s", repoName),
			},
			// REPO_DIGEST is the full by-digest URI of our image. The parameter is populated
			// by app-sre based on probing the image at ${REGISTRY_IMG}:${IMAGE_TAG}.
			{
				Name:     "REPO_DIGEST",
				Required: true,
			},
