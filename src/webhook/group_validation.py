...
def handle_request():
    ...
    try:
        ...
        if userinfo['username'] in ("kube:admin", "system:admin"):
            ...
            if debug:
                print("Performing action: {} in {} group for {}"
                      .format(body_dict['operation'], group_name, userinfo['username']))
        ...
        if group_name.startswith(group_prefix):
            ...
            if len(set(userinfo['groups']) & set(admin_groups)) > 0:
                response_body = responses.response_allow(
                    req=body_dict, msg="{} group {}".format(body_dict['operation'], group_name))
            else:
