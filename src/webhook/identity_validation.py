...
TOTAL_IDENTITY = Counter('webhook_identity_validation_total', 'The total number of identity validation requests')
DENIED_IDENTITY = Counter('webhook_identity_validation_denied', 'The total number of identity validation requests denied')

identity_prefix = os.getenv("IDENTITY_VALIDATION_PREFIX", "OpenShift_SRE")
identity_prefix_list = identity_prefix.split(",")
...
def handle_request():
  ...
  try:
    ...
    if userinfo['username'] in ("kube:admin", "system:admin", "system:serviceaccount:openshift-authentication:oauth-openshift"):
      ...
      if debug:
        print("Performing action: {} on identity {} by {}".format(body_dict['operation'], identity_name, userinfo['username']))
      return responses.response_allow(req=body_dict)
 
    sre_user = False
    for id_prefix in identity_prefix_list:
      if identity_name.startswith(id_prefix):
        sre_user = True
        break

    if sre_user:
      if debug:
        print("Performing action: {} on {} identity".format(body_dict['operation'], identity_name))
      if len(set(userinfo['groups']) & set(admin_groups)) > 0:
        response_body = responses.response_allow(req=body_dict, msg="{} identity {}".format(body_dict['operation'], identity_name))
      else:
        deny_msg = "User not authorized to {} identity {}".format(body_dict['operation'], identity_name)
