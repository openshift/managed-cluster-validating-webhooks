...
parser.add_argument(
    ...
    help='What is the annotation that has a reference to a namespace/configmap for the caBundle. The cert must be '
         'stored in pem format in a key called service-ca.crt')
...
for vwc in get_validating_webhook_configuration_objects_with_annotation(admission_client, parsed.annotation_name):
    ...
    for hook in new_vwc.webhooks:
        if hook.client_config.service is not None and hook.client_config.ca_bundle is not encoded_cert:
            ...
            print("validatingwebhookconfiguration/{new_vwc_name}: Injecting caBundle from {cm_ns}/{cm_name}, for hook "
                  "name {hook_name}, to service/{service_ns}/{service_name}\n".format(
                      new_vwc_name=new_vwc.metadata.name,
                      cm_ns=ns,
                      cm_name=cm_name,
                      hook_name=hook.name,
                      service_ns=hook.client_config.service.namespace,
                      service_name=hook.client_config.service.name))
    try:
