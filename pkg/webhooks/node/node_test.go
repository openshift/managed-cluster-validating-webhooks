...
func runNodeTests(t *testing.T, tests []nodeTests) {
	...
	for _, test := range tests {
		...
		hook := NewWebhook(test.allowCordonNodes)
...
func TestNodes(t *testing.T) {
	tests := []nodeTests{
		...
		{
			allowCordonNodes: true,
			testID:           "node-unpriv-user-update-with-feature-flag",
			targetResource:   "nodes",
			targetKind:       "Node",
			targetVersion:    "v1",
			targetGroup:      "",
...
func TestName(t *testing.T) {
	if NewWebhook(false).Name() == "" {
		t.Fatalf("Empty hook name")
	}
}

func TestRules(t *testing.T) {
	if len(NewWebhook(false).Rules()) == 0 {
		t.Log("No rules for this webhook?")
	}
}

func TestGetURI(t *testing.T) {
	if NewWebhook(false).GetURI()[0] != '/' {
